...
``` {r}
### running the above code with the gotelli propagule rain function instead
prms <- c(ce = 0.15, e = 0.05)
Initial.p <- 0.01
out.L <- data.frame(ode(y = Initial.p, times = 1:100, 
...
out.IMH[["pH"]] <- ode(y = Initial.p, times = 1:100, func = hanski,
                       parms = prms)[, 2] 
  ## adding Hanski values as additional column in df for plots/comparison
```
...
out.IMH[["pH"]] <- ode(y = Initial.p, times = 1:100, func = hanski,
                       parms = prms)[, 2] 
  ## adding Hanski values as additional column in df for plots/comparison
### Casey: adding a Levins model in there too
out.IMH[["levins"]] <- ode(y = Initial.p, times = 1:100, func = levins,
                       parms = prms)[, 2] 
  ## adding Hanski values as additional column in df for plots/comparison

matplot(out.IMH[, 1], out.IMH[, 2:4], type = "l", col = 1,
        ylab = "p", xlab = "time")
legend("topleft", c("Hanski", "Propagule Rain", "Levins"), lty = c(2, 1, 3),
       bty = "n")
```
...
Ds <- c(0, 0.2, 0.5, .6, .7, .8, .9) 
  ### Casey added some extra D values
Initial.p <- 0.05
  ### Casey changed from 0.01 to 0.05 to better see the effects of high D
t <- 1:200
```
...
> Casey note: at high D, population can't keep up and goes extinct.  What is the D value at which the population growth is zero?  Looks like close to $D = 0.6$ here, but changes with initial $p$ (and definitely $c_i$ and $e$ as well).
...
> Casey note: For $c > e$ we'd expect the population to remain stable at 1.  For $c < e$ in the Hanski model, starting at full capacity, the rescue effect part of $ep(1-p)$ means the extinction in any patch is unlikely, so a slow decrease, until $(1-p)$ becomes large enough to allow significant extinctions.  For $c < e$ in the Levins model, the negative $r (= c_i - e)$ term is unabated by the rescue effect so we just get an exponential decline in population.
...
``` {r}
set.seed(1)
out.CS.10 <- MetaSim(method = "hanski", NSims = 10, phi = 10)  
matplot(out.CS.10$t, out.CS.10$Ns, type = "l", xlab = "Time",
        ylab = "Occupancy", sub = out.CS.10$method)
```
> Casey note: `ci` and `e` are parameters that we are leaving at the default, `ci` = `e` = .25: these are the means, with `phi` as the variation.  If `phi` = 0, then this becomes deterministic, and $c_i - e = 0$, so population doesn't change.
...
```{r}
### Casey: let's plot these too
matplot(out.CS.Lots$t, out.CS.Lots$Ns, type = "l", xlab = "Time",
        ylab = "Occupancy", sub = out.CS.Lots$method)
```
...
> Casey note: Obviously the rescue effect is the difference here... when the population occupancy proportion $p \rightarrow 1$ then the Hanski model allows a rescue effect to repopulate any patches that might otherwise have gone extinct, keeping $p$ near 1.  The Levins model does not do this, so as $p \rightarrow 1$, the extinction rate increases ($ep$ term), forcing $p$ downward.
