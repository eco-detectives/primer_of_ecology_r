...
# Did we need to specify t as numeric? Is it not numeric by default?
...
# Why can't I call 'N' and get a vector?
...
```{r}
### Casey's dplyr version
tmax <- 20
growth_df <- data.frame(t = 0:tmax,
                        Nts = dlogistic(t = tmax)) %>%
  mutate(Nts_plus_1 = lead(Nts),
         tot_incr   = Nts_plus_1 - Nts,
         pc_incr    = tot_incr / Nts)

ggplot(growth_df, aes(x = t, y = Nts)) +
  theme_bw() +
  geom_point()

ggplot(growth_df, aes(x = Nts, y = tot_incr)) +
  theme_bw() +
  geom_point()

ggplot(growth_df, aes(x = Nts, y = pc_incr)) +
  theme_bw() +
  geom_point()

# Sean reproducing Casey's version to get up to speed:

# Graphing population growth increment, so this should approach zero, right?

t_max_sd <- 15
growth_df_sd <- data.frame(t = 0:t_max_sd,
                           Nts = dlogistic(t = t_max_sd)) %>% 
  mutate(Nts_plus_1_sd = lead(Nts),
         total_inc_sd = Nts_plus_1_sd - Nts,
         perc_inc_sd = total_inc_sd / Nts)

# Graphing per capita growth increment versus population size:

ggplot(growth_df_sd, aes(x = Nts, y = perc_inc_sd)) +
  geom_point(size = 2, color = "blue") + 
  ggtitle("Per capita growth increment in a density-dependent population") +
  theme_bw() +
  labs(x = "Population size (Nts)", y = "Per capita growth increment")
...
```{r Casey's serious simulation}
n_sims <- 1000
K <- 1/a
t_max <- 15

N0_vec <- c(0, runif(n_sims, 0, 1.2 * K))
# N <- parallel::mclapply(N0s, FUN = function(n) dlogistic(N0 = n), mc.cores = 50)
N_df <- lapply(N0_vec, FUN = function(x) {
  n_vec <- dlogistic(N0 = x, t = t_max)
  return(data.frame(t = 0:t_max,
                    n = n_vec))
}) %>%
  setNames(1:(n_sims+1)) %>%
  bind_rows(.id = 'sim') %>%
...
ggplot(N_df, aes(x = t, y = n, color = sim)) +
  geom_line(aes(group = sim), alpha = .2) +
  scale_color_viridis_c() +
  theme_classic() +
  geom_hline(yintercept = K) +
  geom_text(x = 15, y = (K + 2), label = paste0('K = ', K), 
...
``` {r}
dF_dN <- deriv(pop.growth.rate, c("N", 'r'))
N <- c(0, 1/alpha)
eval(dF_dN)

### Casey's add:
N <- seq(0, 1/alpha, .1/alpha) ### try multiple values of N
eval(dF_dN)
...
> With Casey's addition - how to describe the change in the derivative?  Notes:

$$F(N) = rN(1 - \alpha N)$$
$$\frac{dF(N)}{dN} = r(1 - \alpha N) - rN\alpha$$
