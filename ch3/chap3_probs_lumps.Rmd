...
nts_df <- dlogistic() %>% as.data.frame() %>% rename(Nts=names(.)[1]) %>% mutate(t=c(0:10))
...
*doesn't look right...*
```{r}
dlogistic <- function(alpha = 0.0000007, rd = 3, N0 = 1000, t = 50) {
  N <- c(N0, numeric(t))
  for (i in 1:t) N[i + 1] <- {
      N[i] + rd * N[i] * (1 - alpha * N[i])
    }
...
nts_df <- dlogistic() %>% as.data.frame() %>% rename(Nts=names(.)[1]) %>% mutate(t=c(0:50))
ggplot(nts_df, aes(x = t, y = Nts)) +
  theme_bw() +
  geom_point()
```
...
>>>>>>> c19572dafd222d6954764dd07d92ad8ccbd462ed
...
thetalog <- function(times, y, parms) {
  n <- y[1]
  with(as.list(parms), {
...
# al <- c(0.01)
# 
# out <- ode(y = init.N, times = t.s, thetalog, r = r, theta = th, alpha = al)
```
...
``` {r}
calc_dlog_stoch <- function(N, r_d, K, K_sd = 0) {
...
Run a few iterations at different standard deviations around the same mean
```{r }
N_0 <- 1; r_d <- 1; K <- 100
yrs <- 10
sims <- 10

K_sd_vec <- c(2, 10, 20, 50)
...
print(K_stoch_plot)
print(K_stoch_plot + scale_y_log10() + labs(title = 'logged Y axis'))
```
```
