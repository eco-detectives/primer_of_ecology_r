...
ggplot(expGrw, aes(x = year, y = N)) +
  ...
  labs(x = 'Year', y = 'Population',title = "Hypotehical water lily population - JLC")+
  theme_bw()
```
...
expGrw<-expGrw %>%
  mutate(rate=N/lag(N)) #lag() lags the vector by given (k=) units of obs, default k=1

expGrw$rate[2:nrow(expGrw)] # just to print the growth rates like stevens
...
*The $ on either side gives you 'equation' notation between them. so when you Knit you'll see that N_0 comes out as N with a 0 in the subscript. (Double $$ on either side do the same but isolate  the equation apart from the text.) The backslash is common notation in equations to indicate a symbol. So that will turn out as the symbol for lambda. You also use the backslash for setting up fractions and other equation formatting.*
...
# How do I change the scale on the right so it's not continuous but just reflects the three N0 values?
```
...
projPlt<-ggplot(proj,aes(time,pop))+
  geom_line(aes(lty=factor(r)))+
  facet_wrap(~rateDef,scales = "free")+
  labs(title="Effect of r - JLC",y="N")
...
We know that $N_t = N_0 e^{rt}$. Hence, plugging in for the values in the statement, we get that:

```{r}
rate <- (log(6.3)-log(0.630))/(2003-1700)
```

So the intrinsic rate of growth is `r format(round(rate, 2), nsmall = 2)` new individual per human per year. 
