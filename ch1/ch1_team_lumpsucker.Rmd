...
pop_df2 <- crossing(time, lambda) %>%
  mutate(N_0 = 100,
         N = N_0 * lambda^time,
         lambda = as.factor(lambda)) ### for graphing/grouping later

ggplot(pop_df2, aes(x = time, y = N, group = lambda)) +
  theme_classic() +
  geom_point(aes(shape = lambda, color = lambda)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  annotate("text", x = .5, y = 250, label = expression(lambda > 1), cex = 5) +
  annotate("text", x = .5, y = 50, label = expression(lambda < 1), cex = 5)
```
...
### Calculate the growth rate from year t-1 to year t separately, then average
...
sims_df <- crossing(year = 0:years,
  ...
  mutate(sim_r = sample(obs_r_df$r_obs, n(), replace = TRUE),
         ...
         count = ifelse(year == 0, last(obs_r_df$r_obs), NA)) 
...
pop_sim_one <- function(r_vec = 1.1, n_0 = 100, years = 50) {
