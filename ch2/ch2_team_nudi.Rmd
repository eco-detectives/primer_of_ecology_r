...
### Vincent's 3-stage structured growth function for X years and plots (covers the next chunks before 2.2)
...
project_pop <- function(project = c(0, 0.5, 20, 0.3, 0, 0, 0, 0.5, 0.9), pop = c(100, 250, 50), year=1){
  ...
  A <- matrix(project, nr = 3,
            byrow = TRUE)
  N0 <- matrix(pop, ncol = 1)
  N1 <- A %*% N0
  pop <- N1
  
  temp <- cbind(i,c("Seeds","Small adult", "Large adult"),N1)
  projected <-rbind(projected,temp)
  
  
  }
  
  return_list <- list(N1, as.data.frame(projected))
  
  return(return_list)
    
}

##Ugliest ggplot command

graph1 <- ggplot(project_pop(year=6)[[2]], aes(x=as.numeric(as.character(i)),y=as.numeric(as.character(V3)),color=as.factor(V2))) +
  geom_line() +  
