...
## Vincent was not aware that M*N is not equivalent to M %*% N in R. I believe it's simply M*N in matlab for example 
...
A <- matrix(c(0, 0.5, 20, 0.3, 0, 0, 0, 0.5, 0.9), nr = 3, byrow = T)
N0 <- matrix(c(100, 250, 50), ncol = 1)
...
# Sean: What does the output (above) of one-step stage-structured growth actually mean? Confused by interpretation in text (pg. 39).

# Sean: Other questions:

# 1. Can someone remind me when we use arrows '<-' versus equal sign '=' to make arguments/vectors? Is that right, an argument?

# 2. Is there a keyboard shortcut for the matrix multiplier '%*%' ?
...
### Vincent's N-stage structured growth function for X years and plots
...
project_pop <- function(project = c(0, 0.5, 20, 0.3, 0, 0, 0, 0.5, 0.9), pop = c(100, 250, 50), year=1){
  ...
  return_list <- list(predicted_pop, stage_pop_df, project_matrix)
...
graph1 <- ggplot(project_pop(year=6)[[2]], aes(x=year,y=pop,color=as.factor(stage))) +
  geom_line() +  
...
# Creating a  matrix - cool

N_projection <- matrix(0, nrow = nrow(A), ncol = years + 1)
N_projection [, 1] <- N0

# Run a loop to fill the matrix

for (i in 1:years) {
  N_projection[, i + 1] <- A %*% N_projection[, i]
}
...
# Sean: Did group review this last week?
```
...
Not sure how to do this with R. 

```{r}

#install.packages('igraph', repo='http://nbcgib.uesc.br/mirrors/cran/')
library(igraph)

## Format projection matrix

project_vect <- c(0      , 0      , 0      , 0      , 127    , 4      , 80,
0.6747 , 0.7370 , 0      , 0      , 0      , 0      , 0 ,
0      , 0.0486 , 0.6610 , 0      , 0      , 0      , 0,
0      , 0      , 0.0147 , 0.6907 , 0      , 0      , 0,
0      , 0      ,      0 , 0.0518 , 0      , 0      , 0,
...
number_stages <- sqrt(length(project_vect))


project_matrix <- matrix(project_vect, nr = number_stages, byrow = TRUE)

life_fail <- plot(graph.adjacency(get.adjacency(graph.data.frame(project_matrix),sparse=FALSE)))
...
eig_proj <- eigen(project_matrix)
dom_pos <- which.max(eig_proj$values)

dom_vector <- Re(eig_proj$vectors[,dom_pos])

ssd <- dom_vector/sum(dom_vector)

```

If the projection matrix is constant over time, the population will eventually (stable state concept?) be composed of `r paste0(format(round(ssd[1],2)*100), "%")` of stage 1, `r paste0(format(round(ssd[2],2)*100), "%")` of stage 2, `r paste0(format(round(ssd[3],2)*100), "%")` of stage 3, `r paste0(format(round(ssd[4],2)*100), "%")` of stage 4. `r paste0(format(round(ssd[5],2)*100), "%")` of stage 5, `r paste0(format(round(ssd[6],2)*100), "%")` of stage 6 and `r paste0(format(round(ssd[7],2)*100), "%")` of stage 7.
...
The $\lambda$ is `r format(round(Re(eig_proj$values[dom_pos]), 3), nsmall = 3)`, which represents the asymptotic finite rate of increase for the whole population (?). If guess it requires a constant relative share of stage individuals.  
...
## vincent's regurgitation of the code in the text book without much understanding

M <- eigen(t(project_matrix))
v <- Re(M$vectors[, which.max(Re(M$values))])
RV <- v/v[1]

L1 <- Re(eig_proj$values[dom_pos])

vw.s <- v %*% t(dom_vector)
S <- vw.s/as.numeric(v %*% dom_vector)

S
which(S == max(S), arr.ind = TRUE) #Not right since must be for an actual transition

elas <- (project_matrix/L1) * S
...
We see that the most important stage is the (7,7) element of the elasticity matrix, i.e. surviving the adult stage. Furthermore, it seems that most of the important stage are along the diagonal, which is surviving within stage.
...
If I base myself off of the elasticity estimates, then it should be the last adult stage. 
...
data <- project_pop(project = project_vect, pop = c(0, rep(10,times=6)),year=100)[[2]]

## Better ggplot graph

graph2g <- ggplot(data, aes(x=year,y=pop,color=as.factor(stage))) +
  geom_line() +  
  labs(x = "Year", y = "Stage abundance", color="Stage") +
  theme_classic()
...
data2 <-  project_pop(project = project_vect, pop = c(0, rep(100,times=5),1000),year=100)[[2]]

graph2g2 <- ggplot(data2, aes(x=year,y=pop,color=as.factor(stage))) +
  geom_line() +  
  labs(x = "Year", y = "Stage abundance", color="Stage") +
  theme_classic()
