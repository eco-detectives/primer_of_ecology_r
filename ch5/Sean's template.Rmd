...
```{r}
# What is the within matrix() 'NA' for?
...
N[1, ] <- c(10,10)
for (i in 1:t) N[i + 1, ] <- dlv_comp_2 (N[i, ], alphas)

# Why doesn't this work? dlvcomp2 is an actual function (in the 'primer' library, I believe? So are we not supposed to use the function that we created?)
...
for (i in 1:t) N[i + 1, ] <- dlvcomp2 (N[i, ], alphas) # What exactly are we saying here?
```
...
```{r}
# I don't know what's going on here. I understand we're making a function, and the output is a list of population growth rates for each species, so perhaps I'm mostly just confused about 'parms' and 'as.list.'

clv_comp_2 <- function(t, n, parms) {     # What is 'parms' again? I think it's just a placeholder for a bunch of different parameters (see below). The idea is that you can stipulate these separately while keeping your function the same. But why do this?
...
```{r}
# What is 'ode'? I know we used it in an  earlier chapter but I've since forgotten about it.
