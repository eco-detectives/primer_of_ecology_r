...
dlv_comp_2 <- function(N, alpha, rd = c(1,1)) # N is a vector of length two; alpha is a matrix.
  
# When you specify an argument with "=" (as I do for rd above), you're giving it a default value. You don't have to do this because later you can specify a value, so whether you define the value initially can be a matter of preference. But in general, if you have a rule that you want the function to follow, you can specify it above. Casey's function specified 'round = FALSE' bceause he wanted integer values.
  
# You can add notes within here to help yourself, or be smart (like Casey) and make the argument names indicate the kinds of data you're inputting. For example: N needs to be a vector of 2, alpha is a matrix, and rd is also a vector of 2. Casey has his arguments named 'N_vec', 'alpha_matrix', and 'rd_vec'.
                                             {
...
# We created a function that we will use later. The reason we use brackets to define the alphas is because we're referring to a matrix of alphas that correspond to what is needed in the expressions.
```
...
```{r}
# What is the within matrix() 'NA' for? It specifies that the matrix is empty. (We're creating an empty matrix to fill in with values based on calculations.) But you could also just remove the 'NA,' and you'd get the same thing. It's not necessary.

# How else would we make this matrix? Is their a tidyverse equivalent? Yeah, you could make a dataframe. How would you do this?

N <- matrix(NA, nrow = t + 1, ncol = 2)
N[1, ] <- c(10,10) # This is just saying that for the first row in the matrix N, for all of the columns (which is why we left it blank), make the values match this vector. Note: the number of columns and vector length need to be the same.
for (i in 1:t) {
...
clv_comp_2 <- function(t, n, parms) {    # parms is an argument for the ode function.
  ...
  with(as.list(parms), {
    dn1dt <- r1 * n[1] * (1 - a11 * n[1] - a12 * n[2]) # Why not make a matrix again? # Why do this differently? Is it just because parms includes more than just alpha values?
...
**Something worth noting is that four patterns represent the possible outcomes in the two-species Lotka-Volterra competition model: Species 1 wins in competition, Species 2 wins in competition, coexistence in a stable equilibrium, and competitive exclusion in an unstable equilibrium.**
